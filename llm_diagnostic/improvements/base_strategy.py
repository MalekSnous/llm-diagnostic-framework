...
class BaseImprovementStrategy(ABC):
    ...
    def evaluate_improvement(
        self, baseline_metrics: EvaluationMetrics, improved_metrics: EvaluationMetrics
    ) -> Dict[str, float]:
        """
    ...
    def cost_benefit_analysis(
        self, improvement_delta: Dict[str, float], total_cost: float, key_metric: str = "accuracy"
    ) -> Dict[str, Any]:
        """
    ...
    def compare_strategies(
        self, other_strategies: List["BaseImprovementStrategy"]
    ) -> Dict[str, Any]:
        """
        ...
        for strategy in all_strategies:
            ...
            # Aggregate results
            avg_improvement = sum(
                r.improvement_delta.get("accuracy", 0) for r in strategy.results
            ...
            comparison["strategies"].append(
                {
                    ...
                    "avg_cost": avg_cost,
                    "cost_per_point": (
                        avg_cost / (avg_improvement * 100) if avg_improvement > 0 else float("inf")
                    ),
                }
            )

        # Rank by cost-effectiveness
        comparison["ranking"] = sorted(comparison["strategies"], key=lambda x: x["cost_per_point"])
