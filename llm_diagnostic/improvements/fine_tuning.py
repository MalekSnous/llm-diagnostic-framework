...
# The fine-tuning stack (torch / peft / transformers / datasets) is heavy and
# optional. Import it defensively so the module — and the package — can be
# imported without it; the strategy only needs it when actually run.
try:
    import torch
...
class FineTuningStrategy(BaseImprovementStrategy):
    ...
    def _evaluate_on_test_cases(self, test_cases: List[TestCase]) -> List[TestResult]:
        ...
        for test_case in test_cases:
            ...
            # Evaluate
            success = (
                test_case.expected_output.lower() in prediction.lower()
...
if __name__ == "__main__":
    ...
    test_cases = [TestCase(id="test1", input="What is 3+4?", expected_output="7")]
