...
class StructureValidationTest(BaseFailureTest):
    ...
    def _generate_code_tests(self) -> List[TestCase]:
        ...
        cases.append(
            TestCase(
                ...
                expected_output=None,
                metadata={"output_type": "code", "language": "python", "complexity": "simple"},
            )
        )
        ...
        cases.append(
            TestCase(
                ...
                expected_output=None,
                metadata={"output_type": "code", "language": "python", "complexity": "moderate"},
            )
        )
    ...
    def _generate_format_tests(self) -> List[TestCase]:
        ...
        cases.append(
            TestCase(
                ...
                expected_output=None,
                metadata={"output_type": "format", "format_type": "email", "expected_count": 3},
            )
        )
        ...
        cases.append(
            TestCase(
                ...
                expected_output=None,
                metadata={"output_type": "format", "format_type": "date", "expected_count": 5},
            )
        )
        ...
        cases.append(
            TestCase(
                ...
                expected_output=None,
                metadata={"output_type": "format", "format_type": "phone", "expected_count": 3},
            )
        )
    ...
    def _evaluate_json(
        ...
        metrics = {"output_type": "json", "complexity": test_case.metadata["complexity"]}
    ...
    def _evaluate_code(
        ...
        metrics = {"output_type": "code", "language": test_case.metadata["language"]}
    ...
    def _evaluate_format(
        ...
        metrics = {"output_type": "format", "format_type": format_type}
    ...
    def get_diagnostic_insights(self) -> Dict[str, Any]:
        ...
        for result in self.results:
            ...
            if output_type not in by_type:
                by_type[output_type] = {"success": [], "details": []}
        ...
        type_analysis = {
            t: sum(data["success"]) / len(data["success"]) for t, data in by_type.items()
        }

        # JSON-specific analysis
        json_results = [r for r in self.results if r.metrics.get("output_type") == "json"]
        if json_results:
            json_valid_rate = sum(r.metrics.get("valid_json", 0) for r in json_results) / len(
                json_results
            )
            json_completeness = sum(
                r.metrics.get("field_completeness", 0) for r in json_results
            ) / len(json_results)
        else:
            json_valid_rate = 0.0
