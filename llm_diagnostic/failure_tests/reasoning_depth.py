...
class ReasoningDepthTest(BaseFailureTest):
    ...
    def _generate_logical_deduction(self, depth: int) -> Tuple[str, str]:
        ...
        statements = []
        items = random.sample(
            ["dogs", "mammals", "animals", "living things", "cats", "pets", "creatures", "beings"],
            depth + 1,
        )
    ...
    def get_diagnostic_insights(self) -> Dict[str, Any]:
        ...
        depth_analysis = {depth: sum(scores) / len(scores) for depth, scores in by_depth.items()}

        task_analysis = {task: sum(scores) / len(scores) for task, scores in by_task_type.items()}
