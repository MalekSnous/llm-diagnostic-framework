...
class HallucinationPatternsTest(BaseFailureTest):
    ...
    def evaluate_single(
        ...
        uncertainty_phrases = [
            "i don't know", "i'm not sure", "i cannot", "i don't have information",
            "no information", "not familiar", "cannot verify", "unable to confirm"
        ]
        ...
        if test_type == "entity" and test_case.metadata.get("is_fake"):
            ...
            if context_type == "irrelevant":
                ...
                identified_contradiction = any(word in prediction_lower for word in contradiction_words)
        ...
        if test_case.context:
            hal_metrics = Evaluator.hallucination_detection(
                [prediction],
                [test_case.context]
            )
    ...
    def get_diagnostic_insights(self) -> Dict[str, Any]:
        ...
        for result in self.results:
            ...
            by_type[test_type]["hallucination_risk"].append(result.metrics.get("hallucination_risk", 0))
        ...
        type_analysis = {
            t: {
                ...
                "avg_hallucination_risk": sum(data["hallucination_risk"]) / len(data["hallucination_risk"]),
        ...
        overall_risk = sum(r.metrics.get("hallucination_risk", 0.0) for r in self.results) / len(self.results) if self.results else 0.0
        ...
        uncertainty_rate = sum(r.metrics.get("expressed_uncertainty", 0.0) for r in self.results) / len(self.results) if self.results else 0.0
