...
class KnowledgeBoundariesTest(BaseFailureTest):
    ...
    def evaluate_single(
        ...
        uncertainty_phrases = [
            ...
            "not available", "cannot provide", "after my knowledge cutoff",
            "beyond my training", "i cannot answer"
        ]
    ...
    def get_diagnostic_insights(self) -> Dict[str, Any]:
        ...
        for result in self.results:
            ...
            by_category[category]["uncertain"].append(result.metrics.get("expressed_uncertainty", 0))
            by_category[category]["expected_to_know"].append(result.metrics.get("expected_to_know", 0))
        
        # Calculate averages
        type_analysis = {
            t: sum(scores) / len(scores)
            for t, scores in by_test_type.items()
        }
        ...
        category_analysis = {
            cat: {
                ...
                "was_expected_to_know": sum(data["expected_to_know"]) / len(data["expected_to_know"])
            }
        ...
        hallucination_cases = [
            ...
            if r.metrics.get("expected_to_know", 0.0) == 0.0 and r.metrics.get("expressed_uncertainty", 0.0) == 0.0
        ]
