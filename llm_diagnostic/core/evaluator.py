...
class Evaluator:
    ...
    def classification_metrics(
        ...
        # Calculate metrics
        metrics.add_metric("precision", precision_score(ref_numeric, pred_numeric, average="weighted", zero_division=0))
        metrics.add_metric("recall", recall_score(ref_numeric, pred_numeric, average="weighted", zero_division=0))
        metrics.add_metric("f1", f1_score(ref_numeric, pred_numeric, average="weighted", zero_division=0))
        ...
        for i, pred in enumerate(predictions):
            try:
                ...
                parse_errors.append({
                    "index": i,
                    "error": str(e),
                    "output": pred[:200]  # First 200 chars
                })
