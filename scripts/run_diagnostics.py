...
def run_single_test(
    test_class, test_name, llm_client, num_cases=10, output_dir="./results/diagnostics"
):
    ...
    # Run test (results are stored on the test instance)
    test.run_test(llm_client, verbose=True)
...
Examples:
  ...
  python scripts/run_diagnostics.py --model claude-3-sonnet-20240229 --test all
    ...
    parser.add_argument(
        "--num-cases", type=int, default=10, help="Number of test cases per test (default: 10)"
    )
    ...
    parser.add_argument(
        "--max-tokens", type=int, default=500, help="Max tokens for LLM responses (default: 500)"
    )
    ...
    for _test_key, (test_class, test_name) in tests_to_run:
        try:
            ...
            all_results[test_name] = {"aggregated": aggregated, "insights": insights}
    ...
    for test_name, result in all_results.items():
        ...
        summary_table.add_row(
            test_name, f"{agg['success_rate']:.1%}", f"${agg['total_cost_usd']:.4f}", bottleneck
        )
