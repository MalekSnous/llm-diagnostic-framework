...
def display_improvement_results(result, strategy_name):
    ...
    for metric_name in result.baseline_metrics.metrics:
        ...
        table.add_row(
            metric_name.capitalize(), f"{baseline_val:.3f}", f"{improved_val:.3f}", delta_str
        )
...
def compare_strategies(results_dict):
    ...
    for strategy_name, result in results_dict.items():
        ...
        table.add_row(
            strategy_name, f"+{acc_delta:.1f}%", f"${cost:.4f}", f"{time:.1f}s", cost_per_point_str
        )
    ...
    if results_dict:
        ...
        console.print("  Best cost-effectiveness for this task")
...
Examples:
  ...
  python scripts/run_improvements.py --strategy all --baseline diagnostic_results.json
    ...
    parser.add_argument(
        "--baseline", type=str, required=True, help="Path to baseline diagnostic results JSON file"
    )
    ...
    parser.add_argument("--kb", type=str, help="Knowledge base file (for RAG strategy)")
    ...
    if args.strategy == "rag" or args.strategy == "all":
        if args.kb:
            ...
            result = run_rag_system(test_cases, baseline_results, llm_client, knowledge_base)
            if result:
    ...
    if args.strategy == "fine-tune":
        console.print(
            "[yellow]Fine-tuning requires separate setup. Use fine_tuning.py directly.[/yellow]"
        )
        ...
        for strategy_name, result in results_dict.items():
            ...
            result_data = {
                ...
                "cost_benefit_analysis": {
                    ...
                    "cost_per_point": result.total_cost
                    / (result.improvement_delta.get("accuracy", 0.001) * 100),
                },
            }
