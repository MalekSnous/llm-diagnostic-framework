...
from llm_diagnostic.core.evaluator import Evaluator, EvaluationMetrics
...
EXPECTED_ENTITIES = [
    ...
    ["migraine", "sumatriptan"],
     ["chest pain", "myocardial infarction", "hypertension", "diabetes", "hyperlipidemia", "aspirin", "metoprolol", "lisinopril", "atorvastatin"],
    ["gravida 2 para 1", "38 weeks", "rupture of membranes", "fetal heart rate", "cervix", "electronic fetal monitoring"],
...
def run_baseline(test_cases, llm_client):
    ...
    for test_case in test_cases:
        ...
        entities_found = sum(1 for entity in EXPECTED_ENTITIES[int(test_case.id.split('_')[1])] 
        ...
        baseline_results.append(TestResult(
            test_case_id=test_case.id,
            prediction=prediction,
            reference=test_case.expected_output,
            success=accuracy > 0.5,
            metrics={"accuracy": accuracy, "entities_found": entities_found, "total_entities": total_entities},
            response=response
        ))
...
def run_prompt_engineering(test_cases, baseline_results, llm_client):
    ...
    console.print("\n[bold cyan]Step 2: Applying Prompt Engineering[/bold cyan]")
      ...
      # 🔍 DEBUG : Vérifier le client
    console.print(f"[yellow]🔍 Client type: {type(llm_client).__name__}[/yellow]")
    console.print(f"[yellow]🔍 Client model: {llm_client.model_name}[/yellow]")
    ...
    if improved_results and improved_results[0].response:
        console.print(f"[yellow]🔍 First response cost: ${improved_results[0].response.cost_usd:.4f}[/yellow]")
        console.print(f"[yellow]🔍 First response model: {improved_results[0].response.model}[/yellow]")
    ...
    # Recalculer l'accuracy manuellement
    total_accuracy = 0
    ...
    total_cost = sum(r.response.cost_usd for r in improved_results if r.response and r.response.cost_usd)
    ...
    # console.print(f"[green]Improved Accuracy: {improved_accuracy:.1%} ({improvement:+.1f}%)[/green]")
    # console.print(f"[green]Cost: $0.0000[/green]")
    ...
    # Créer les objets EvaluationMetrics correctement
...
def generate_summary(baseline_acc, baseline_cost, prompt_result, rag_result):
    ...
    prompt_cost_per_point = prompt_result.total_cost / prompt_improvement if prompt_improvement > 0 else float('inf')
    ...
    rag_cost_per_point = rag_result.total_cost / rag_improvement if rag_improvement > 0 else float('inf')
...
def main():
    ...
    if api_key:
        ...
        console.print(f"[red]✗ No API Key found![/red]")
        return
